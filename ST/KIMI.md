...
- **Prefer `StrReplaceFile` over `WriteFile`** when editing existing files. Only use `WriteFile` for creating new files or complete rewrites.
- **Use parallel tool calls aggressively.** Read multiple files, run multiple searches, or inspect multiple locations in a single turn whenever possible.
- **Use `Agent` with `subagent_type="explore"`** for any read-only codebase exploration that would require more than 3 search queries. Launch multiple explore agents concurrently for independent questions.
- **Use `EnterPlanMode`** before starting non-trivial implementation tasks (new features, multi-file changes, architectural decisions, or unclear requirements).
- **Use `SetTodoList`** to track progress on tasks with multiple subtasks or milestones.
- **Use `Shell(run_in_background=true)`** for long-running commands (builds, tests, watchers). Manage them via `/task`.
- **Follow the docs hierarchy:** start with `Documentation/README.md`, then `Documentation/session_start.md`, then `Documentation/project_memory.md`, then the relevant component docs.

## Project Memory

Persistent project-state notes live in `Documentation/project_memory.md`.
Use it as the durable handoff file for "what changed and what matters now" between sessions.
Superseded plans and stale reference docs should be moved to `Documentation/archive/` when added.
...
- **`_arrival`** — Collision arrival sanity-check (ON-SCREEN / OFF-SCREEN / IMPLODE)
- **`_dist_warn`** — Distance-increase error corrections
- **`_foreshadow`** — Approaching/imminent/converging collision foreshadow nudge
- **`_intimacy`** — (Phase 2: retained slot, now unused — cleared every turn)
...
- **Arrival decision gate**: When a collision hits distance 0, the extension injects a single-turn sanity-check block asking the LLM to commit ON-SCREEN, OFF-SCREEN (REFRAME or DISSOLVE), or IMPLODE — all resolutions complete that turn. Tracked via `_firedCollisionArrivals` Set in `index.js`.
