...
The live prompt slot is the generic `_challenge` slot. The older `_combat` slot is kept clear by `index.js` so stale combat packets do not overlap with the challenge-engine packet.
