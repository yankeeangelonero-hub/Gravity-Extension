...
- The former split preset has been promoted/renamed to `gravity_v14.json` so the modular preset + World Info path is clearly versioned.
- Additional prose-style toggles now exist in both `gravity_v13_c.json` and `gravity_v14.json`: `Glass Nerve`, `Lyrical Ruin`, and `Street Voltage`, with `Noir Realist` still left enabled by default.
